...
fn test_rk4_exp_growth() {
    ...
    let exact = 1.0_f64.exp();
    let err = (y[0] - exact).abs();
    // RK4 global error = O(h^4). With h=0.1: error ≈ (e/120)*h^4 ≈ 2.3e-6
    // Use 1e-4 as conservative bound (100x margin over theoretical estimate)
    assert!(err < 1e-4, "RK4 exp growth error too large: {err}");
}
...
fn test_rk4_harmonic() {
    ...
    let err_x = (y[0] - exact_x).abs();
    let err_v = (y[1] - exact_v).abs();
    // RK4 global error = O(h^4). With h=0.1 and 2D system: error ≈ h^4/120 ≈ 8e-7
    // Use 1e-4 as conservative bound (100x margin over theoretical estimate)
    assert!(err_x < 1e-4, "RK4 harmonic x error: {err_x}");
    assert!(err_v < 1e-4, "RK4 harmonic v error: {err_v}");
}
